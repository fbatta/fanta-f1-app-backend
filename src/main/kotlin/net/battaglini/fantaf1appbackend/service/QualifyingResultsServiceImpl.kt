...
@Service
class QualifyingResultsServiceImpl(
    private val openF1Client: OpenF1Client,
    private val driverService: DriverService
) : QualifyingResultsService {
    override suspend fun getDriversResultsForQualifying(
        ...
        return driverService.getDriversInSessions(listOf(sessionKey)).map { driver ->
            ...
            DriverQualifyingResult(
                ...
                fastestLapQ1 = result?.duration?.getOrNull(0)?.toDuration(DurationUnit.SECONDS)
                    ?: 9_999.toDuration(DurationUnit.SECONDS),
                fastestLapQ2 = result?.duration?.getOrNull(1)?.toDuration(DurationUnit.SECONDS)
                    ?: 9_999.toDuration(DurationUnit.SECONDS),
    ...
    companion object {
        private val LOGGER = LoggerFactory.getLogger(QualifyingResultsServiceImpl::class.java)
    }
}